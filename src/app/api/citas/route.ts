...
export async function GET(request: Request) {
  ...
  if (email) {
    return NextResponse.json(getCitasByEmail(email));
  }
  ...
  if (year && month && duracion) {
    const days = getAvailableDays(Number(year), Number(month), Number(duracion));
  ...
  return NextResponse.json(getCitas(filters));
}
...
export async function POST(request: Request) {
  ...
  if (body.action === "reprogramar") {
    ...
    if (!isSlotAvailable(fecha, hora, cita.servicio_duracion, cita.id)) {
      return NextResponse.json({ error: "Horario no disponible" }, { status: 409 });
    }

    reprogramarCita(Number(id), fecha_hora);
    return NextResponse.json({ success: true });
  }

  if (body.action === "cancelar") {
    updateCitaEstado(Number(body.id), "cancelada");
    return NextResponse.json({ success: true });
  }
  ...
  if (body.action === "estado") {
    ...
    updateCitaEstado(Number(body.id), body.estado as EstadoCita);
    return NextResponse.json({ success: true });
  }
  ...
  const fecha = fecha_hora.split("T")[0];
  const hora = fecha_hora.split("T")[1]?.slice(0, 5);
  const servicio = (await import("@/lib/db")).getServicioById(Number(servicio_id));
  if (!servicio) return NextResponse.json({ error: "Servicio no encontrado" }, { status: 404 });

  if (!isSlotAvailable(fecha, hora, servicio.duracion_minutos)) {
    return NextResponse.json({ error: "Horario no disponible" }, { status: 409 });
  }

  const clienteId = getOrCreateCliente(
    parsed.data.nombre,
    parsed.data.email,
    parsed.data.telefono
  );

  const citaId = createCita({
