...
export async function exportCitaComprobante(cita: {
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.section}>
          ...
          <Text>{cita.servicio_nombre} — {cita.servicio_precio.toFixed(2)} €</Text>
        </View>
        <View style={styles.section}>
          <Text style={styles.label}>Cliente</Text>
          <Text>{cita.cliente_nombre}</Text>
          <Text>{cita.cliente_email}</Text>
          <Text>{cita.cliente_telefono}</Text>
        </View>
        <View style={styles.section}>
          <Text style={styles.label}>Fecha y hora</Text>
          <Text>{new Date(cita.fecha_hora).toLocaleString("es-ES")}</Text>
        </View>
        <View style={styles.section}>
  ...
  const blob = await pdf(Doc).toBlob();
  downloadBlob(blob, `cita_${cita.id}.pdf`);
}
